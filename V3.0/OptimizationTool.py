...
class OptimizationTool(object):
	...
	def _obj_func_MLE_no_PRS_A_facto(self,x):
		...
		val = np.asarray(self.simulator.do_direct_sim(x,self.count,len(self.target_list),self.count,obj))
		#print(val)
		...
		for i,case in enumerate(self.target_list):
			if case.target == "Tig":
				...
				#val = self.somefunction(x) #Similar to Tig case
		...
		self.count +=1
		diff = np.asarray(direct_sim_value)-np.asarray(target_value)
	...
	def run_optimization_with_MLE_no_PRS(self,Unsrt_data,Input_data):
		...
		if Input_data["Stats"]["Design_of_PRS"] == "A-facto":
			...
			start = time.time()
			opt_output = minimize(self.obj_func_of_direct_simulations_3_param,self.init_guess,bounds=bounds,method='Powell',options={"maxfev":500000})
			stop = time.time()
