...
{self.jpdap_executable} jpdap_data_{self.rIndex}.txt &> out"""
		...
		print(f"\n\tJPDAP code took {stop, 0.01, 0.02 , Jpdap-start_Jpdap}s to execute\n")
		Nagy_covariance_matrix = ""
		file_name_jpdap = "jpdap_data_"+str(self.rIndex)+".txt_fit_minRMSD.txt"
		Nagy_covariance_matrix = open(file_name_jpdap,"r").readlines()[5:8]
		covariance_matrix = ""
...
class thermodynamic:
	...
	def uncertPriorObjective (self,guess):
		...
		if "h" in self.sub_type:
			...
			residual_h =  np.array([0.01, 0.020])
		if "e" in self.sub_type:
...
class collision:
	def __init__(self, Element,string,mechanism_loc):
		...
		plt.ylabel('Uncertainity ($\sigma$)')
		plt.title( string+'{}'.format(self.name), fontsize = 10)		
		plt.plot(self.temperatures,self.thermodynamicuncertainties,'o',label='exp uncertainties');
		plt.ylim(0,2*max(self.uncertainties[0],self.uncertainties[-1]))	
		plt.legend()
		my_path = os.getcwd()
		plt.savefig(my_path+'/Plots/TDeptUnsrt/'+self.name+'.png')
...
#Parent class to host the uncertainty data for al0.01, 0.02l the reactions. Has instances of reaction class as members		
	...
	def __init__(self,pathDictionary,binary_files,unsrt_type=None):
		...
		for child in self.root:
			...
			if child.tag == 'fallOffCurve':
				...
				count_foc +=1
			if child.tag == 'thermo':
				NASA_L = thermodynamic(child,self.thermoPath,"Low")
				NASA_H = thermodynamic(child,self.thermoPath,"High")
				self.thermoList.append(NASA_L.nametag)
				self.thermoList.append(NASA_H.nametag)
				self.unsrt_data[NASA_L.nametag] = NASA_L
				self.unsrt_data[NASA_H.nametag] = NASA_H
				count_thermo +=1
