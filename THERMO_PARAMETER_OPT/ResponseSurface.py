...
class ResponseSurface(object):
	...
	def create_response_surface(self):
		...
		if 'responsecoef_case-'+str(self.case_index)+'.csv' in os.listdir(os.getcwd()+'/Data/ResponseSurface/'):
			...
			#print(np.shape(self.BTrsMatrix))
			#print(np.shape(self.X))
			#print(np.shape(self.Y))
			#U, S, Vt = np.linalg.svd(self.BTrsMatrix, full_matrices=False)
			...
			#self.coeff = np.linalg.solve(self.R,np.transpose(y)) ### this was causing error so i changed to below 
			# Solve using least-squares
			self.coeff, _, _, _ = np.linalg.lstsq(self.R, np.transpose(y), rcond=None)
			#self.coeff = Vt.T @ np.linalg.inv(np.diag(S)) @ U.T @ y
			#print(self.coeff)
			"""
			Writing the response surface for checking the co-efficients
			"""
