...
def process_dm_zeta(input_vector):
    ...
    theta_low = [ 500, 600, 700, 800, 900]
    theta_high= [1000, 1100, 1200, 1300, 1400]
...
def calculate_z_vector(
    ...
    scaled_nominal = np.dot(cov_matrix, nominal_param)
    ...
    Cp_Nominal = np.dot(Theta, scaled_nominal)
    
    # Denominator max (Max Cp/R at all 5 temperatures)
    Cp_Max = np.dot(Theta, scaled_max)

    # Numerator target (Target Cp/R at all 5 temperatures)
    Cp_Target = np.dot(Theta, scaled_target)
    ...
    numerator = Cp_Target - Cp_Nominal
    
    # Denominator: Maximum Possible Deviation from Nominal
    denominator = Cp_Max - Cp_Nominal
...
def main():
    ...
    output_file = "dm_actual_params.csv"
