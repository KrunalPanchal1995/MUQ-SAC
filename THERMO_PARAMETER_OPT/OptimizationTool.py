...
class OptimizationTool(object):
	...
	def fitness_function_for_T_DIPENDENT(self, ga_instance, solution, solution_idx):
		#print("entering the fitness function..linr 258")
		global obj_func_of_selected_PRS_thermo
		def obj_func_of_selected_PRS_thermo(Z,solution_idx):	
			#print("DBG enter obj_func_of_selected_PRS_thermo | sol_idx=", solution_idx, "Z_len=", None if Z is None else len(Z))
			self.count +=1
			string_x = "{self.count},"
			for i in Z:
				string_x+=f"{i},"
			...
			## add the constraint here to check iterations
			# 1. Check the derivative constraint
			
			min_derivative = self.cons_6_derivative_positive(V)
			#print("min derivative line 344", min_derivative)
			if min_derivative < -1e-5:
				fitness = 1/10**5
				print("constrained voilated 347",fitness)
				return fitness
			else:
				...
				print("printing fitness line 501",fitness)
				print(solution, solution_idx)
				#raise AssertionError
				return fitness
		try:
			fitness = obj_func_of_selected_PRS_thermo(solution, solution_idx)
			if fitness is None:
				print("Try function..507  DBG: fitness is None -> returning -1e12")
				raise AssertionError
			return fitness
		except Exception as e:
			print("Exception,,,,511 DBG: exception in fitness wrapper:", repr(e))
			raise AssertionError						
			return obj_func_of_selected_PRS_thermo(solution, solution_idx)
	...
	#Has to be changed for current case
	'''
'''
	...
	def run_optimization_with_selected_PRS_thermo(self,unsrt_data,ResponseSurfaces,Input_data):
		...
		if Input_data["Stats"]["Design_of_PRS"] == "A-facto":
			...
			if "GD" in self.opt_method:
				...
				#print("entering the GA function else part..line 1354")
				fitness_function = self.fitness_function_for_T_DIPENDENT
				
				#self.init_guess = np.zeros(len(self.rxn_index))
				gene_space = [{'low': -1, 'high': 1} for _ in self.init_guess ]
				
				#gene_constraint = lamda x,v:[val for val in v if self.cons_6_derivative_positive(val)>=0]
...
# origional script without modifications
...
# global variable to keep valid constraints count
valid_constraints_count = 0
DESIGN_MATRIX_PATH = "/data2/RANA/FINAL_RUN_7nc/FULL_RUN_1/dm_Z_values.csv"
class OptimizationTool(object):
	def __init__(self,
		target_list=None,frequency = None,opt_method = "GA"):
	...
	def cons_6_derivative_positive(self, V):
		_species_order_enforced = [
			"AR_Low", "AR_High", "H2_Low", "H2_High", "O_Low", "O_High",
			"O2_Low", "O2_High", "H2O_Low", "H2O_High", "CO_Low", "CO_High",
			"C_Low", "C_High", "HCO_Low", "HCO_High", "OH*_Low", "OH*_High",
		...
		for speciesID in _species_order_enforced:
			...
			if speciesID == "O_Low" or speciesID == "O_High":
				...
				# Slice the `z` vector to get the 5 parameters for the current species.
				z_species = V[start_index : start_index + 5]
				...
				for T in temperatures_points:
					...
					# Perturb the nominal parameters (p_o) using the genetic algorithm parameters (z_species)
					# and the covariance matrix (cov).
					p_modified = p_o + cov @ z_species                   ### being stored in the main excecutive folder
	...
	def fitness_function_for_T_DIPENDENT(self, ga_instance, solution, solution_idx):
		#global obj_func_of_selected_PRS_thermo
		#def obj_func_of_selected_PRS_thermo(solution,solution_idx):	
		Z = solution
		...
		string+=f"\n"
		V = np.asarray(V)
		#print("transformed solution passed to constraints: \n ", V)
		min_derivative = self.cons_6_derivative_positive(V) # stored in GA_optimization folder
		...
		print("min derivative:", min_derivative)
		
		
		if min_derivative < -1e-3:
			fitness = 1/(10**9)
			#print("constrained voilated 347",fitness)
			#return fitness			
		else:
			self.valid_constraints_count += 1
			print("constraints satisfies count \t ", self.valid_constraints_count)
			#return fitness
		...
		try:
			...
			print("Exception,,, DBG: exception in fitness wrapper:", repr(e))
			raise AssertionError						
			return obj_func_of_selected_PRS_thermo(solution, solution_idx)	'''
	...
	def load_initial_population_from_csv(self,file_path, num_solutions=1):
		"""
		Reads the design matrix CSV, extracts the last 'num_solutions' rows,
		and returns them as a NumPy array suitable for pygad's initial_population.
		"""
		...
		try:
			...
			# 2. Verify Data Size
			if df.shape[0] < num_solutions:
				print(f"⚠️ Warning: File has only {df.shape[0]} rows. Using all available rows.")
				population_df = df
			else:
				# 3. Select the LAST 'num_solutions' rows (the latest iterations)
				population_df = df.iloc[-num_solutions:]

			# 4. Convert to a NumPy array (pygad requires this)
			initial_population_array = population_df.values.astype(np.float64)

			print(f"✅ Successfully loaded initial population.")
			print(f"	Shape of initial_population: {initial_population_array.shape}")
	...
	def run_optimization_with_selected_PRS_thermo(self,unsrt_data,ResponseSurfaces,Input_data):
		...
		if Input_data["Stats"]["Design_of_PRS"] == "A-facto":
			...
			#print(self.cov_dict)
			#raise AssertionError("Zeta_max values")
			#print(len(self.T))
			self.init_guess = np.zeros(len(self.T)*len(self.species_index))
			#self.init_guess = np.array([-2.96893286,0.76861981,17.4261154,45.4912758,10.0504961,-1.50947742,1.07985437,1.65623474,-0.339644179,5.14789303,1.41980216,-2.7868245,18.5771791,6.38730047,2.03608777,0.735639342,0.635280217,4.44022129,2.46345247,1.08053651,-1.0320216,2.28972601,11.2433774,8.84643232,2.6845374,-3.32188469,-5.5823844,-4.73678754,-24.621364,4.49043001,0.109785722,-3.61280577,72.268569,37.1318507,8.58929532,4.56276155,16.72694,-20.3222704,-22.4714367,11.6492897,2.83900928,2.98787738,-35.8113613,-14.0451364,-2.11520168,11.8108703,-14.8673447,-61.017312,117.928411,27.3207054,-0.512431326,-0.395861123,14.5678042,8.11913429,2.5010311,-1.00499721,-0.540682717,-0.323527367,-0.0752015412,1.21455104,0.192446569,0.694958936,34.9679687,4.32339462,-78.2844219,-0.266273536,-0.171549093,-6.13088537,-6.90337571,5.25188327,18.8685928,346.641921,-1778.20726,3886.08529,1763.37944,-9.38066425,-32.1146073,-181.403055,-11.5461109,56.3705034,3.11977583,1.62205554,-21.3389812,-5.59264694,-0.230593311,6.90737712,-40.2357966,-12.208475,-151.828192,-23.9850608,1.09325274,-0.405419652,-8.22169336,5.77094053,1.35098526,1.10570741,4.32684794,2.39707177,1.62472607,11.7328336])
			#self.init_guess = np.ones(len(self.T)*len(self.species_index))
			cons = []
			n_vectors = len(self.init_guess)
			...
			if "GD" in self.opt_method:
				...
				# --- NEW CODE: Data Loading and Population Sizing ---
				NUM_SEEDED_SOLUTIONS = 1000
				ORIGINAL_RANDOM_POP = 400

				# 1. Load the data using your function (defined elsewhere)
				initial_pop_data = self.load_initial_population_from_csv(
					DESIGN_MATRIX_PATH,
					num_solutions=NUM_SEEDED_SOLUTIONS
				)

				# 2. Determine the argument for pygad
				if initial_pop_data is None:
					# If load failed, revert to original population size (400 random)
					initial_population_arg = None
					TOTAL_POP_SIZE = ORIGINAL_RANDOM_POP
				else:
					initial_population_arg = initial_pop_data
					# Set total population size to the loaded count + the original random count
					TOTAL_POP_SIZE = initial_pop_data.shape[0] + ORIGINAL_RANDOM_POP	
				...
				self.ga_instance = pygad.GA(num_generations=2000,
									...
									# *** MODIFIED PARAMETERS *** ### comment it out if you dont want to give initail population
									sol_per_pop=400,
									initial_population=initial_population_arg, # The loaded data
