...
gas = ct.Solution('/data/TEST-THERMO-sens/FFCM_THERMO_UQ_OPT/PRS_INVESTIGATION_ID_1032_TIG/Perturbed_Mech/mechanism_0.yaml')

reactorTemperature = 1165.0 #Kelvin
reactorPressure = 1605000.0

global criteria
criteria = reactorTemperature+50


gas.TPX = reactorTemperature, reactorPressure,{'H2': 0.00088, 'CO': 0.01654, 'O2': 0.0174, 'AR': 0.96518}
...
def ignitionDelay(df,pList, species,cond="max",specified_value ="None;None",exp_conc = "None"):
	if cond == "max":
		...
		#print(valid_indices)
		#smoothed_oh_star = gaussian_filter1d(df[species].to_numpy()[valid_indices], sigma=5)
		#fig = plt.figure()
		#plt.plot(df.index.to_numpy(),df[species].to_numpy())
		#plt.plot(df.index.to_numpy()[valid_indices],df[species].to_numpy()[valid_indices],"r--")
		#plt.plot(df.index.to_numpy()[valid_indices],smoothed_oh_star,"b-")
		#plt.show()
