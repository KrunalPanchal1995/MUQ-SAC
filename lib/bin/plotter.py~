...
def plot_optResults(dataSet):
	...
	if "Tig" in dataSet.dataType[0].strip():
		...
		plt.yscale('log')
		#plt.title('P =  {} and Phi = {}'.format(dataSet.Pressure[0],temp_phi[0]), fontsize = 10)
		#print(dataSet.BG1)
		if dataSet.BG2_conc[0] == "":
			
			plt.text(-30,20 , ' {}% {}/{}%{}/{}%{}\nP = {} Pa'.format(float(dataSet.Fuel_conc[0])*100,dataSet.Fuel[0],float(dataSet.Oxidizer_conc[0])*100,dataSet.Oxidizer[0],float(dataSet.BG1_conc[0])*100,dataSet.BG1[0],dataSet.Pressure[0]), fontsize = 22)
			
		else:
		
			plt.text(-30,20 , ' {}% {}/{}% {}/{}% {}/{}% {}\nP = {}'.format(float(dataSet.Fuel_conc[0])*100,dataSet.Fuel[0],float(dataSet.Oxidizer_conc[0])*100,dataSet.Oxidizer[0],float(dataSet.BG1_conc[0])*100,dataSet.BG1[0],float(dataSet.BG2_conc[0])*100,dataSet.BG2[0],dataSet.Pressure[0]), fontsize = 22)
  
		plt.plot(1000.0/np.asarray(dataSet.Temperature),np.asarray(dataSet.observed),'o',label = "Observed")
		plt.errorbar(1000.0/np.asarray(dataSet.Temperature),np.asarray(dataSet.observed),yerr = np.asarray(dataSet.Unsrt_fct),fmt='o',label = "$\sigma_{exp}$")
		plt.plot(1000.0/np.asarray(dataSet.Temperature),(np.asarray(dataSet.nominalSimulations)),'--',label = "A. Keromens et al. (2013)")
		plt.plot(1000.0/np.asarray(dataSet.Temperature),(np.asarray(dataSet.opt_target_values)),'-',label = "Current work")
		plt.legend()
		#plt.show()
		plt.savefig('Data/Tig/'+str(dataSet.tag)+'.pdf')
		plt.close()
...
def plot_uncertFunc(variable):
	if variable.dataKey.strip() != "":
		for i in variable.dataKey.split(","):
			...
			plt.plot((1/T),u,'o',label='Uncertainties (Baulch (2002))')
			plt.plot((1/T),u_k,"-",label='Uncertainty function ($\sigma_{\kappa}$)')
			plt.plot((1/T),u_z,"--",label='Uncertainty function (change of base ;$\sigma_{\zeta}$)')
		...
		plt.rcParams['font.sans-serif'] = ['sans-serif']
		plt.rcParams['axes.unicode_minus'] = False
		plt.savefig("./Plots/"+str(variable.tag)+"/uncert_func/"+str(variable.name)+".pdf")
		plt.legend()
		plt.close()
...
def plot_uncertLimits(variable):
	...
	if variable.dataKey.strip() != "":	
		for i in variable.dataKey.split(","):
			...
			plt.plot((1/T),N,"-",label='Nominal')
			plt.plot((1/T),N_upper,"b-",label='Upper uncertainty limit ($\sigma_{\kappa}$)')
			plt.plot((1/T),N_lower,"b-",label='Lower uncertainty limit ($\sigma_{\kappa}$)')
			plt.plot((1/T),N_basisLower,"r--",label='Upper uncer tainty limit ($\sigma_{\zeta}$)')
...
def plot_optimized(variable):
	...
	if variable.dataKey.strip() != "":	
		for i in variable.dataKey.split(","):
			...
			plt.plot((1/T),N,"b--",label='Nominal')
			plt.plot((1/T),N_opt,"r-",label="Optimized")
			plt.plot((1/T),N_basisLower,"b--",label='Upper uncertainty limit ($\sigma_{\zeta}$)')
			plt.plot((1/T),N_basisUpper,"b--",label='Lower uncertainty limit ($\sigma_{\zeta}$)')
			plt.rcParams['font.sans-serif'] = ['sans-serif']
			plt.rcParams['axes.unicode_minus'] = False		
		...
		plt.rcParams['font.sans-serif'] = ['sans-serif']
		plt.rcParams['axes.unicode_minus'] = False
		plt.savefig("./Plots/"+str(variable.tag)+"/optimized/"+str(variable.name)+".pdf")
		plt.close()
		plt.legend()
