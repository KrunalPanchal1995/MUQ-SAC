...
class Parser:
    ...
    def read_NASA7_entry(self, lines, TintDefault, comments):
        ...
        text = lines[0][45:55]
        match = re.search(r"(?:\b\w?\s+)?(\d+(?:\.\d+)?)\b", text)
        ...
        if match:
            ...
            raise AssertionError("Tmin was not read properly!!(Krunal)")
        Tmin = fortFloat(number)
    ...
    def read_kinetics_entry(self, entry, surface):
        ...
        #For PLOG purpose - k_inf: (author: Krunal Panchal)
        A_inf = A
        b_inf = b
        Ea_inf = Ea
        ...
        #Will work if only reaction is PLOG type
        if rxn_type == 'PLOG':
            pdep_arrhenius.append([10000.000, arrhenius])
