...
"""for rxn_id in sensitivity_dict:
	...
	sensitivity_data.append(float(sensitivity_dict[rxn_id][700]))
	##plt.plot(Temperature,sensitivity_data,"-",label=f"{rxn_id}")
	plt.barh(rxn_id, sensitivity_data, color=['green' if v > 0 else 'red' for v in sensitivity_data])"""
for rxn_id in sensitivity_dict:
    if 700 in sensitivity_dict[rxn_id]:  
        sensitivity_value = sensitivity_dict[rxn_id][700]
        plt.barh(rxn_id, sensitivity_value, color='green' if sensitivity_value > 0 else 'red')
plt.axvline(x=0, color='black', linestyle='--', linewidth=1)
plt.title("Diverging Bar Plot: Positive and Negative Values")
...
plt.savefig("SA1.pdf",bbox_inches="tight")
