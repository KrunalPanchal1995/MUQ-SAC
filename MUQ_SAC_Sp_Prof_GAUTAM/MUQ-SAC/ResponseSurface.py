...
class ResponseSurface(object):
	...
	def plot(self, index, slice_no, slice_type):				
		plot_dir = f'../Plots/case_{self.case_index}'
		...
		# Plot training data
		plt.scatter(np.asarray(self.Y), np.asarray(self.resFramWrk), color="none", edgecolor="green",
				    label=f"Training (max error = {self.MaxError:.3f}%, mean error = {self.MeanError:.3f}%)")

		# Ensure parity line is exactly at 45 degrees
		min_val = min(min(self.Y), min(self.resFramWrk)) * 0.98
		max_val = max(max(self.Y), max(self.resFramWrk)) * 1.02
		plt.xlim(min_val, max_val)
		plt.ylim(min_val, max_val)
		
		# Parity line at 45 degrees
		x = np.linspace(min_val, max_val, 1000)
		plt.plot(x, x, "-", label="Parity line", color="blue",linewidth=0.5)
		...
		# Save as PDF
		plot_path = os.path.join(plot_dir, f'Parity_plot_case_{self.case_index}_{slice_type}{slice_no}_training.pdf')
	...
	def create_response_surface(self):
		...
		if os.getcwd()+'/Data/ResponseSurface/responsecoef_case-'+str(self.case_index)+'.csv' is True:
			...
			self.BTrsMatrix = self.MatPolyFitTransform()  #This takes in the PRS matrix and finds the entries of the matrix (206*55)
			rows = len(self.BTrsMatrix)  # Number of rows
			cols = len(self.BTrsMatrix[0])
			#print(rows,cols) #Rows remain constant but 55 cols because 1 bias+ 9 linear term + 9 direct squared terms and nC2 cross relation terms(36)
			#print(np.shape(self.X))
			self.Q, self.R = np.linalg.qr(self.BTrsMatrix) #QR decomposition of BTrsMatrix  (206,55) and (55,55)
			#print(np.shape(self.Q))
			#print(np.shape(self.R))
			y = np.dot(np.transpose(self.Q),self.Y) #(55,206) and (206,)--> (55,)
			self.coeff = np.linalg.solve(self.R,np.transpose(y))  #Solves using Back Substitution 
			#print(len(self.coeff))  #55
			"""
			Writing the response surface for checking the co-efficients
			"""
	...
	def evaluate_prs(self,x):
		...
		#print(tow)-->1
		if tow > 0:
			for i,j in enumerate(BZeta): #(index,value)
				for k in BZeta[i:]:
					if count < len(coeff):					
						val+=coeff[count]*j*k
						count +=1
	...
	def resCoeffTransform(self,order):
		...
		zero.append(coeff[0])
		count = 1
		if tow > 0:		#Extracting the Linear terms
			for _ in row:				
				a.append(coeff[count])
				count +=1
			tow = tow - 1

		if tow > 0:   #Extracting second order and cross relational terms
			for i,j in enumerate(row): 
				temp = []
