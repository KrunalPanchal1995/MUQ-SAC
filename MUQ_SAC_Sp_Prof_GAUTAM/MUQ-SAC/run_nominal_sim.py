...
for case in case_dir:	
    ...
    if "sim_data_case-"+str(case)+".lst" in os.listdir():
        ...
        h = '../Data/Simulations/Species_Profile.csv'
        
        if case == 0:
            species_conc.columns = [f'time(us)_case-{case}', f'OH_Concentration(mol/cm3)_case-{case}']
            species_conc.to_csv(h, index=False)
        else:
            existing_data = pd.read_csv(h)
            species_conc.columns = [f'time(us)_case-{case}', f'OH_Concentration(mol/cm3)_case-{case}']
            blank_space = pd.DataFrame(np.nan, index=existing_data.index, columns=[''])
            updated_data = pd.concat([existing_data, blank_space, species_conc], axis=1)
            updated_data.to_csv(h, index=False)
