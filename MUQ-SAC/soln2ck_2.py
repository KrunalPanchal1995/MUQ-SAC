...
def write(solution, factor, fname):
    ...
    with open(output_file_name, 'w+') as f:
        ...
        def build_modified_arrhenius(equation_object, t_range, uf):
            ...
            coeff_sum = sum(equation_object.reactants.values())
            if t_range == 'high':
                pre_exponential_factor = equation_object.rate.high_rate.pre_exponential_factor*uf
                temperature_exponent = '{:.3f}'.format(equation_object.rate.high_rate.temperature_exponent)
                activation_energy = '{:.2f}'.format(equation_object.rate.high_rate.activation_energy/calories_constant)
                if coeff_sum == 1:
                    pre_exponential_factor = str(
                                    '{:.3E}'.format(pre_exponential_factor))
                if coeff_sum == 2:
            ...
            if t_range == 'low':
                #print(dir(equation_object))
                #print(dir(equation_object.rate.low_rate))
                pre_exponential_factor = equation_object.rate.low_rate.pre_exponential_factor*uf
                temperature_exponent = '{:.3f}'.format(equation_object.rate.low_rate.temperature_exponent)
                activation_energy = '{:.2f}'.format(equation_object.rate.low_rate.activation_energy/calories_constant)
        ...
        for reac_index in range(len(trimmed_solution.reaction_equations())):
            ...
            if equation_type == 'FalloffReaction':
                ...
                j = equation_object.rate.falloff_coeffs
                ...
                try:
                    ...
                    pass
                #trimms efficiencies list
                #print(dir(equation_object.rate))
                efficiencies = equation_object.efficiencies
                trimmed_efficiencies = equation_object.efficiencies
                for s in efficiencies:
                    if s not in trimmed_solution.species_names:
                ...
                if bool(efficiencies) is True:
                    f.write(fourth_line)
            #dupluicate option
            #print(dir(equation_object.duplicate))
