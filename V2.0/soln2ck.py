...
def species_data_text(species_list, note):
    ...
    if note:
        ...
        species_names = [f"{s:<{max_species_len}}\n" for s in species_list]
        species_names = fill(
            ''.join(species_names), 
...
def write(solution, output_path='', input_yaml='',
    ...
    with open(output_path, 'w') as mech_file:
        ...
        for n, reaction in enumerate(solution.reactions()):
            ...
            #print(f"\t\t{count}\t{reaction_equation}\n")
            ...
            if type(reaction) == ct.FalloffReaction:
                ...
                #print(reaction.rate.falloff_coeffs.size)
                # need to print additional falloff parameters if present
                if reaction.rate.falloff_coeffs.size > 0:
                    falloff_str = build_falloff(reaction.rate.falloff_coeffs, reaction.rate.type)
                    width = max_rxn_width - 10 - 15*(reaction.rate.falloff_coeffs.size - 3)
                ...
                # need to print additional falloff parameters if present
                print(dir(reaction.rate.falloff_coeffs))
                raise AssertionError("Stop!!")
                if reaction.falloff.parameters.size > 0:
                    falloff_str = build_falloff(reaction.rate.falloff_coeffs, reaction.falloff.type)
    ...
    if not skip_thermo and not same_file_thermo:
        ...
        with open(therm_path, 'w') as thermo_file:
            thermo_file.write(thermo_data_text(solution.species(), input_type='file',note=""))
...
write(gas, output_path='mechanism.inp', input_yaml='mechanism.yaml',skip_thermo=False, same_file_thermo=False, skip_transport=False)
a = deepcopy(open("mechanism.inp","r").read())
b = open("mechanism.mech","w", encoding='ascii').write(a+"\n")
#c = open("mechanism.mech","w+", encoding='ascii').write("\n")
