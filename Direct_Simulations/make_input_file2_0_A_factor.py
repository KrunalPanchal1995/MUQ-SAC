...
def ignitionDelay(df,species,cond="max",specified_value ="None;None"):
	...
	if cond == "max":
		...
		concentration = profile[max_index]
		...
		if len(peaks) > 0:
			...
			tau_max = t[first_peak_index]
			conc_max = profile[first_peak_index]
			onset_def = 0.05*conc_max
			index,value = find_nearest(profile,onset_def)
			tau = t[index]
...
tau,concentration = ignitionDelay(df,ignitionDelayDefination,ign_cond,specific)
file_d2 = open("spec_conc","w+")
file_d2.write("spec_conc\\t{{}}\\tmol/cm3".format(concentration))
file_d2.close()

file_d3 = open("tau.out","w+")
file_d3.write("tau\\t{{}}\\t(ms)".format(tau))
file_d3.close()
saveAll = {flag}
if saveAll == False:
	def delete_files(directory):
		# Patterns for files to delete
		patterns = ["*.out", "*.csv", "*.zip"]
...
""".format(molecular_weight=target.add["mol_wt"],ign_def = target.add["ign_delay_def"],cond = target.add["ign_cond"],specific=target.add["specific_cond"], flag = target.add["saveAll"])
		else:
...
rm -f ScanMan.log
		...
		s_run = """#!/bin/bash
python3.9 /home/krithika/MUQ-SAC/V3.0/soln2ck_FM.py --mechanism=mechanism.inp --thermo=therm.dat --transport=trans.dat {mech} &> soln2ck.log
{Bin}/ScanMan -i mechanism.inp -t therm.dat -m trans.dat -f chemkin -3sr -N 0.05 -E -o mechanism.pre &> ScanMan.log
{Bin}/FlameMan &> Flame.log && python3.9 extract.py &> extract.log
find ./output -type f ! -name 'tau.out' ! -name 'modified.csv' -delete &> delete.log
