...
def run_map(params,total):
	...
	subprocess.call(["cp",params[-1],location])#copies pre file for FM simulations
...
def run_direct_map(file_dict):
	...
	#with open(location+"/mechanism.yaml","w") as yamlfile:
	#	yamlfile.write(yaml_string)
	#print(file_dict["simulationInputString"])
	yamlwriter.dump_to_yaml(location,f"mechanism.yaml",file_dict["mechanism"])
...
class SM(object):
	...
	def getSimulationFiles(self,iter_number,cases,beta):
		self.simulation = "Opt"
	...
	def getSimulatedValues(self,beta,iter_number,cases,objective):

		#Prior_Mechanism = Parser(self.mech_loc).mech
		#self.copy_of_mech = copy.deepcopy(Prior_Mechanism)
		print(f"Starting direct simulations: Iteration number {iter_number}, total cases to simulate: {cases}")
		dictionary_list,mkdir_list,dir_run_list,run_convert_list,output_list = self.getSimulationFiles(iter_number,cases,beta)
	...
	def do_direct_sim(self,beta,iter_number,targets,objective):
		cases = len(targets)
		directSimulation = self.getSimulatedValues(beta,iter_number,cases,objective)
