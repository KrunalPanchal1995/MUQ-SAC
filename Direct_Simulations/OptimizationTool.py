...
class OptimizationTool(object):
	...
	def fitness_function_for_T_INDIPENDENT(self):
		...
		def fitness_func_T_indi(x,solution_idx):
			...
			# Direct Simulation Part
			val = np.asarray(self.simulator.do_direct_sim(x,self.count,self.target_list,obj))		
			direct_sim_value.append(val)
			#print(val)
			
			for i,case in enumerate(self.target_list):	
				if case.target == "Tig" and case.add['op_type']=='species_profile':
					exp_df = pd.read_csv(case.add['exp_profile'],delimiter=',',header=None)
					x_column = 0 # Index of the column for the x-axis (e.g., first column)
					y_column = 1 # Index of the column for the y-axis (e.g., second column)
					time = exp_df[x_column]
					conc = exp_df[y_column]	 
					time_exp,conc_exp = find_time(time,conc)
					#val = self.ResponseSurfaces[i].evaluate(x)
					COUNT_All+=1
					COUNT_Tig+=1
					count_slice_time[i] = 0
					count_slice_conc[i] = 0
					for idx,time in enumerate(time_exp): #The key will have all 14 time slices of responses  					
						std_dvtn = 0.1*time_exp[idx]
						#print(np.exp(response.evaluate(x))/10, std_dvtn, time_exp[idx])
						#print(np.exp(response.evaluate(x))/10,time_exp[idx])
						response_value.append(val[i][0][idx]*1000/(std_dvtn)) #sec to ms
						target_value.append(time_exp[idx]/std_dvtn)
						count_slice_time[i]+=1
							
					for idx,conc in enumerate(conc_exp): #The key will have all 14 time slices of responses  
				
						#print(np.exp(response.evaluate(x))*1e-3,conc_exp[idx])												
						std_dvtn = 0.1*conc_exp[idx]
						#print(np.exp(response.evaluate(x))*1e-4, std_dvtn, conc_exp[idx])
						response_value.append(val[i][1][idx]*1000000/std_dvtn) #mol/m^3 to PPM
						target_value.append(conc_exp[idx]/std_dvtn)						
						count_slice_conc[i]+=1
						
					#print(response_value)
					#print(target_value)
					#raise AssertionError("Opt loop check")
				elif case.target == "Tig" and case.add['op_type']!='species_profile':
					COUNT_All +=1
					COUNT_Tig +=1
					#dataset_weights = (1/len(self.frequency))*float(self.frequency[case.d_set])
					#dataset_weights = (1/COUNT_All)
					...
					#print(val)
					#val,grad = case.evaluateResponse(x)
					response_value.append(val[i]/(np.log(case.std_dvtn*10)))
					#response_stvd.append(grad)
					target_value.append(np.log(case.observed*10)/(np.log(case.std_dvtn*10)))
			...
			for i,case in enumerate(self.target_list):			
				if case.target == "Tig" and case.add['op_type']=='species_profile':
					...
					time_exp,conc_exp = find_time(time,conc)
					for idx,time in enumerate(time_exp): #The key will have all 14 time slices of responses  
						multiplicating_factors.append((1/COUNT_Tig)*(1/count_slice_time[i]))
					for idx,conc in enumerate(conc_exp): #The key will have all 14 time slices of responses  
						multiplicating_factors.append((1/COUNT_Tig)*(1/count_slice_conc[i]))
				elif case.target == "Tig" and case.add['op_type']!='species_profile':			
					multiplicating_factors.append(1/COUNT_Tig)
			...
			for i in x:
				string_x +=f"{i},"
			note = open("guess_values.txt","+a").write(f"{self.count},{string_x}\n")
			string_target_value = ""
			string_direct_sim_value = ""
			for i in target_value:
				string_target_value+=f"{i},"
			string_target_value+="\n"
			for i in direct_sim_value:
				string_direct_sim_value+=f"{i},"
			string_direct_sim_value+="\n"
			get_target_value = open("response_values.txt","+a").write(string_direct_sim_value)
			get_target_value = open("target_values.txt","+a").write(string_target_value)
			get_opt = open("Objective.txt","+a").write(f"{obj}\n")
	...
	def _obj_func(self,x):
		...
		for i,case in enumerate(self.target_list):	
			if case.target == "Tig" and case.add['op_type']=='species_profile':
				...
				count_slice_time[i] = 0
				count_slice_conc[i] = 0
				for idx,response in enumerate(self.ResponseSurfaces[i]['time_slice']): #The key will have all 14 time slices of responses  
					if response.selection == 1:
						std_dvtn = 0.1*time_exp[idx]
						#print(np.exp(response.evaluate(x))/10, std_dvtn, time_exp[idx])
						#print(np.exp(response.evaluate(x))/10,time_exp[idx])
						response_value.append(np.exp(response.evaluate(x))/(10*std_dvtn))
						target_value.append(time_exp[idx]/std_dvtn)
						count_slice_time[i]+=1
						
				for idx,response in enumerate(self.ResponseSurfaces[i]['species_slice']): #The key will have all 14 time slices of responses  
					if response.selection == 1:
						#print(np.exp(response.evaluate(x))*1e-3,conc_exp[idx])												
						std_dvtn = 0.1*conc_exp[idx]
						#print(np.exp(response.evaluate(x))*1e-4, std_dvtn, conc_exp[idx])
						response_value.append(np.exp(response.evaluate(x))*1e-4/std_dvtn)
						target_value.append(conc_exp[idx]/std_dvtn)						
						count_slice_conc[i]+=1
		...
		for i,case in enumerate(self.target_list):	
			if case.target == "Tig" and case.add['op_type']=='species_profile':
				...
				for idx,time in enumerate(time_exp): #The key will have all 14 time slices of responses  					
					...
					#print(np.exp(response.evaluate(x))/10, std_dvtn, time_exp[idx])
					#print(np.exp(response.evaluate(x))/10,time_exp[idx])
					response_value.append(val[i][0][idx]/(std_dvtn))
					target_value.append(time_exp[idx]/std_dvtn)
					count_slice_time[i]+=1
				...
				for idx,conc in enumerate(conc_exp): #The key will have all 14 time slices of responses  
					...
					std_dvtn = 0.1*conc_exp[idx]
					#print(np.exp(response.evaluate(x))*1e-4, std_dvtn, conc_exp[idx])
					response_value.append(val[i][1][idx]/std_dvtn)
					target_value.append(conc_exp[idx]/std_dvtn)						
					count_slice_conc[i]+=1
	...
	def run_optimization_with_DirectSimulation(self,Unsrt_data,Input_data):
		...
		design_matrix = [[1]]
		self.simulator = simulator(self.target_list,self.Input_data,self.unsrt,design_matrix,tag="Full")
		if Input_data["Stats"]["Design_of_PRS"] == "A-facto":
			...
			opt_output = minimize(self.direct_obj_func,self.init_guess,bounds=bounds,method=algorithm)
			print(opt_output)
			...
			#on_generation=lambda ga_instance: early_stopper.check(ga_instance),
			...
			"""
			opt_output = minimize(self._obj_func_MLE_no_PRS_A_facto,
					    self.init_guess,
					    bounds=bounds,
					    method=algorithm,  # Replace 'algorithm' with specific method if known
			...
			optimal_parameters_zeta = np.asarray(optimal_parameters)
			cov = []
